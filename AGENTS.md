...
### Worker 라벨은 화면에서 바꿀 수 있어야 한다

채널이 운영 중에 생기므로, 새 채널의 일을 받으려고 재시작하게 만들면 안 된다.
`Worker`의 `labels`가 배열이 아니라 함수인 건 이 때문이다 — 매 claim마다 읽어간다.

바꾼 값은 별도 파일(`~/.samdi/worker-labels.json`)에 남긴다. **사용자의 YAML을 프로그램이
